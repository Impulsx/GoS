...
if target == nil then return end	
	if IsValid(target) then
		...
		local time = myHero.pos:DistanceTo(target.pos) / (1500+myHero.ms) 
		...
		if myHero.pos:DistanceTo(target.pos) <= 950 and myHero.pos:DistanceTo(target.pos) > 300 and not MarkBuff and Ready(_R) and Ready(_Q) and QDmg*2 < target.health then
			self:CastR(target)
		end	
		...
		if not myHero:GetSpellData(_E).name == "IreliaE2" then 
    ...
    local minion = GameMinion(i)
		...
		if myHero.pos:DistanceTo(minion.pos) <= 600 and minion.team == TEAM_ENEMY and IsValid(minion) then
			...
			if self.Menu.ClearSet.LastHit.UseQ:Value() and myHero.mana/myHero.maxMana >= self.Menu.ClearSet.LastHit.Mana:Value() / 100 and Ready(_Q) then
				...
				local time = myHero.pos:DistanceTo(minion.pos) / (1500+myHero.ms) 
				local hp = CheckHPPred(minion)

				if QDmg > minion.health and hp > 0 and not IsUnderTurret(minion) and not myHero.isDashing then	
					ControlCastSpell(HK_Q, minion)
				end	

				if QDmg > minion.health and hp > 0 and IsUnderTurret(minion) and AllyMinionUnderTower() and not myHero.isDashing then
					ControlCastSpell(HK_Q, minion)
...
if GotBuff(myHero, "ireliapassivestacksmax") == 1 then return end	
    ...
    local minion = GameMinion(i)
		...
		if myHero.pos:DistanceTo(minion.pos) <= 600 and minion.team == TEAM_ENEMY and IsValid(minion) then
			...
			local time = myHero.pos:DistanceTo(minion.pos) / (1500+myHero.ms) 
			local hp = CheckHPPred(minion)
				if QDmg > minion.health and hp > 0 and not myHero.isDashing then
					ControlCastSpell(HK_Q, minion)
...
function Irelia:CastQMinion(target)
    ...
    local minion = GameMinion(i)
		...
		if myHero.pos:DistanceTo(minion.pos) <= 600 and minion.team == TEAM_ENEMY and IsValid(minion) then
			...
			local time = myHero.pos:DistanceTo(minion.pos) / (1500+myHero.ms) 
			local hp = CheckHPPred(minion)
			if myHero.pos:DistanceTo(minion.pos) > myHero.pos:DistanceTo(target.pos) and not IsUnderTurret(minion) and target.health > Dmg and QDmg > minion.health and hp > 0 and not myHero.isDashing then
				ControlCastSpell(HK_Q, minion)
		...
		if target.pos:DistanceTo(myHero.pos) < 1000 then
			if Ready(_Q) then
				...
				local minion = GameMinion(i)
					...
					local time = myHero.pos:DistanceTo(minion.pos) / (1500+myHero.ms) 
					local hp = CheckHPPred(minion)
						if target.pos:DistanceTo(myHero.pos) < minion.pos:DistanceTo(target.pos) and QDmg > minion.health and hp > 0 and not myHero.isDashing then
							ControlCastSpell(HK_Q, minion)
						end
					end	
                end
