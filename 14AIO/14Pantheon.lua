...
function Pantheon:__init()
    ...
    self.Q2 = {Hitchance = GGPrediction.HITCHANCE_HIGH, Type = GGPrediction.SPELLTYPE_LINE, Delay = 0, Radius = 60, Range = 1200, Speed = 1500, Collision = true, MaxCollision = 0, CollisionTypes = {GGPrediction.COLLISION_YASUOWALL}}
    self.W = {Range = 600}
    self.E = {Range = 250}
    self.R = {Range = 5500}
...
function Pantheon:CastQ1(target)
    ...
    and orbwalker:CanMove() and GetDistanceSquared(myHero.pos,target.pos) < self.Q1.Range ^2 then
        ...
        if Pred:CanHit(self.Q1.Hitchance or GGPrediction.HITCHANCE_HIGH) then
...
function Pantheon:CastQ2(target)
    ...
    and self.Qchannel and Ready(_Q)  and  lastQup + 150 < GetTickCount() then
        ...
        if Pred:CanHit(self.Q2.Hitchance or GGPrediction.HITCHANCE_HIGH) then
