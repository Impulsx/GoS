...
local function CheckMinionCollisionLocal(source, endPos, delay, radius, speed, range, start)
...
local function CheckMinionCollision(source, endPos, delay, radius, speed, range, start)
	if _G.SDK and _G.SDK.Orbwalker then
		...
		return CheckMinionCollisionLocal(source, endPos, delay, radius, speed, range, start)
	end
end
...
----------------------------------------------------------------------
	...
	QData = {Type = _G.SPELLTYPE_LINE, Delay = (0.25+ping), Radius = 50, Range = 1100, Speed = 1300, Collision = true, MaxCollision = 0, CollisionTypes = {_G.COLLISION_MINION}}
	QspellData = {speed = 1300, range = 1100, delay = (0.25+ping), radius = 50, collision = {"minion"}, type = "linear"}

	QNoColData = {Type = _G.SPELLTYPE_LINE, Delay = (0.25+ping), Radius = 45, Range = 1100, Speed = 2100, Collision = false}
	QNoColspellData = {speed = 2100, range = 1100, delay = (0.25+ping), radius = 45, collision = {nil}, type = "linear"}

	WData = {Type = _G.SPELLTYPE_LINE, Delay = (0.25+ping), Radius = 87, Range = 1105, Speed = 1700, Collision = false}
	WspellData = {speed = 1700, range = 1105, delay = (0.25+ping), radius = 87, collision = {nil}, type = "linear"}

	EData = {Type = _G.SPELLTYPE_CIRCLE, Delay = (0.25+ping)+0.55, Radius = 185, Range = 800, Speed = MathHuge, Collision = false}
	EspellData = {speed = MathHuge, range = 800, delay = (0.25+ping)+0.55, radius = 185, collision = {nil}, type = "circular"}
end

function LoadSpells()

	Q = {Range = 1100, Width = 50,Delay = 0.25, Speed = 1300}
	Q2 = {Range = 1100, Width = 45,Delay = 0, Speed = 2100}
	W = {Range = 1105, Width = 87,Delay = 0.25, Speed = 1700}
	E = {Range = 800, Width = 185,Delay = 0.25+0.55, Speed = math.huge}
		...
		if enemy ~= nil and IsValid(enemy) then
			if GetDistance(myHero.pos, enemy.pos) <= Menu.Skills.Q.Range:Value() and not CheckMinionCollision(myHero, enemy.pos, Q.Delay, Q.Width, Q.Speed, Q.Range, myHero.pos) then
				...
				elseif Menu.Pred.Change:Value() == 2 then
					...
					local QPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = (0.25+ping), Radius = 50, Range = 1100, Speed = 1300, Collision = true, CollisionTypes = {GGPrediction.COLLISION_MINION}})
			...
			if PredPos == false then
				...
				elseif Menu.Pred.Change:Value() == 2 then
					...
					local QPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = (0.25+ping), Radius = 50, Range = 1100, Speed = 2100, Collision = false})
					QPrediction:GetPrediction(enemy, myHero)
	...
	if Menu.Skills.W.UseW:Value() then
		...
		if enemy ~= nil and IsValid(enemy) then
			...
			elseif Menu.Pred.Change:Value() == 2 then
				...
				local WPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = (0.25+ping), Radius = 87, Range = 1105, Speed = 1700, Collision = false})
	...
	if Menu.Skills.E.UseE:Value() then
		...
		if enemy ~= nil and IsValid(enemy) then
			...
			elseif Menu.Pred.Change:Value() == 2 then
				...
				local EPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = (0.25+ping)+0.55, Radius = 185, Range = 800, Speed = MathHuge, Collision = false})
