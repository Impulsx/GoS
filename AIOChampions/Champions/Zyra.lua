...
if target == nil then return end		
	if IsValid(target) and myHero.pos:DistanceTo(target.pos) <= 850 and Ready(_W) then
...
if target == nil then return end
	if IsValid(target) then
		...
		if myHero.pos:DistanceTo(target.pos) <= 850 and Menu.Combo.UseW:Value() and Ready(_W) then
			Control.CastSpell(HK_W, target.pos)
			DelayAction(function() 
			Control.CastSpell(HK_W, target.pos) 
			end, 0.05)
...
function Clear()
    ...
    local minion = Game.Minion(i)
		...
		if minion.team == TEAM_ENEMY and IsValid(minion, 1200) and myHero.mana/myHero.maxMana >= Menu.Clear.Mana:Value() / 100 then					
...
function JungleClear()
    ...
    local minion = Game.Minion(i)	
		...
		if minion.team == TEAM_JUNGLE and IsValid(minion, 1200) and myHero.mana/myHero.maxMana >= Menu.JClear.Mana:Value() / 100 then	
