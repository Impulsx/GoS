...
function Combo()
	...
	local target = GetTarget(QData.Range+100)
...
function CastQ(target)
	...
	if GetDistance(target.pos, myHero.pos) < QData.Range and myHero:GetSpellData(_Q).toggleState == 1 then
		...
		elseif Menu.Pred.Change:Value() == 2 then
			...
			local QPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.25, Radius = 110, Range = 1100, Speed = 850, Collision = false})
