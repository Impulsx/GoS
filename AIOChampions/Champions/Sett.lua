...
function LoadScript()
	...
	Menu.Clear:MenuElement({ id = "Wmin", name = "[W] If Hit X Minion ", value = 2, min = 1, max = 6, step = 1, identifier =
	"Minion/s" })
	...
	Menu.Clear:MenuElement({ id = "Emin", name = "[E] If Hit X Minion ", value = 3, min = 1, max = 6, step = 1, identifier =
	"Minion/s" })
	...
	Menu:MenuElement({ type = MENU, id = "ManlyW", name = "Manual [W] Menu" })
	Menu.ManlyW:MenuElement({ id = "ManW", name = "Manual [W] Key", key = string.byte("T") })
	Menu.ManlyW:MenuElement({ id = "GritToggle", name = "[W] Grit Toggle", value = true })
	Menu.ManlyW:MenuElement({ id = "Grit", name = "Min Grit to Use [W]", value = 50, min = 0, max = 100, identifier = "%" })



	WData =
	{
		Type = _G.SPELLTYPE_LINE, Delay = 0.75, Radius = 60, Range = 750 + (myHero.boundingRadius / 2), Speed = MathHuge, Collision = false
	}

	WspellData = { speed = WData.Speed, range = WData.Range, delay = WData.Delay, radius = WData.Radius, collision = { nil }, type =
	"linear" }

	EData =
	{
		Type = _G.SPELLTYPE_LINE, Delay = 0.25, Radius = 175, Range = 450 + (myHero.boundingRadius / 2), Speed = MathHuge, Collision = false
	}

	EspellData = { speed = EData.Speed, range = EData.Range, delay = EData.Delay, radius = EData.Radius, collision = { nil }, type =
	"linear" }
	...
	Callback.Add("Draw", function()
		...
		if Menu.Drawing.DrawW:Value() and Ready(_W) then
			DrawCircle(myHero, WData.Range, 1, DrawColor(225, 225, 0, 10))
		end
		if Menu.Drawing.DrawE:Value() and Ready(_E) then
			DrawCircle(myHero, EData.Range, 1, DrawColor(225, 225, 0, 10))
		end
...
function Tick()
	...
	local Mode = GetMode()
	if Menu.ManlyW.ManW:Value() then
		Orbwalk()
		ManualW()
	end
	if Mode == "Combo" then
...
function ManualW()
	local target = GetTarget(WData.Range)
	if target == nil then return end
	local canAttack = CanAttack()
	if myHero.pos:DistanceTo(target.pos) < WData.Range and Ready(_W) and canAttack then
		if Menu.ManlyW.GritToggle:Value() and not (myHero.mana / myHero.maxMana >= Menu.ManlyW.Grit:Value() / 100) then
			return
		end
		if Menu.Pred.Change:Value() == 1 then
			local pred = GetGamsteronPrediction(target, WData, myHero)
...
function CastGGPredW(unit)
	local WPrediction = GGPrediction:SpellPrediction({ WData.Delay, Radius = WData.Radius, Range = WData.Range, Speed =
	WData.Speed, Collision = false, Type = GGPrediction.SPELLTYPE_LINE })
...
function CastGGPredE(unit)
	local EPrediction = GGPrediction:SpellPrediction({ EData.Delay, Radius = EData.Radius, Range = EData.Range, Speed =
	EData.Speed, Collision = false, Type = GGPrediction.SPELLTYPE_LINE })
